...
PRUN_Options = []
if ROOTCOREBIN: PRUN_Options = ["--useRootCore" , "--cmtConfig=%s" % ROOTCORECONFIG , "--writeInputToTxt=\"IN:input.txt\"" , "--rootVer=" + ROOTVER]
elif ATLASPROJECT.startswith("Analysis"): PRUN_Options = [ "--cmtConfig=%s" % PLATFORM, "--writeInputToTxt=\"IN:input.txt\""]
TarList = []
if CREATETARBALL:
    TarList = ["--extFile=*.root", "--outTarBall=TarBall.tgz", "--excludeFile='*.svn*','*.git*','*.pyc','\*.*~','\*.tex','\*.tmp','\*.pdf','\*.png','\*.log','\*.dat','\*.core','\*README*'"]
    if ROOTCOREBIN or ATLASPROJECT.startswith("Analysis"): TarList.append(" --workDir='%s'" % (WORKAREA))
...
def setupGridSubmitArgParser():
    ...
    if ROOTCOREBIN or ATLASPROJECT.startswith("Analysis"): parser.add_argument('--exe', help='executable/run script to use for submitting (give package name and scipt name, e.g. XAMPPbase/python/runSUSYAnalysis.py)', default='XAMPPbase/python/runSUSYAnalysis.py')
    ...
    parser.add_argument('--FilesPerJob', help='You can specify the number of files per job... If you have data then it would be may be better to give a larger number... Default: 1', type=int, default=1)
...
def SubmitJobs(RunOptions, AnalysisOptions):
   ...
   # check if inputDS was a file
    ...
    ######################################
    if (ROOTCOREBIN or ATLASPROJECT.startswith("Analysis")) and (RunOptions.IsPrimAOD or isPrimaryAOD(InputDataSets)): AnalysisOptions.append("--IsPrimAOD")
    ...
    ############################################
    if ROOTCOREBIN or ATLASPROJECT.startswith("Analysis"):
    ...
    if ROOTCOREBIN or ATLASPROJECT.startswith("Analysis"): prettyPrint('Executable', RunOptions.exe)
