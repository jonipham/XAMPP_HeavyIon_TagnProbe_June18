...
StatusCode SUSYObjDef_xAOD::FillMuon(xAOD::Muon& input, float ptcut, float etacut) {
  ...
  //ATH_MSG_FATAL("imput.muontype() = " << input.muonType());
  //if (input.muonType() == xAOD::Muon::SiliconAssociatedForwardMuon) ATH_MSG_FATAL("xAOD::Muon::SiliconAssociatedForwardMuon" );
  //if (input.muonType() == xAOD::Muon::Combined) ATH_MSG_FATAL("xAOD::Muon::Combined" );
  //if (input.muonType() == xAOD::Muon::MuonStandAlone) ATH_MSG_FATAL("xAOD::Muon::MuonStandAlone" );
  //if (input.muonType() == xAOD::Muon::SegmentTagged) ATH_MSG_FATAL("xAOD::Muon::SegmentTagged" );
  //if (input.muonType() == xAOD::Muon::CaloTagged) ATH_MSG_FATAL("xAOD::Muon::CaloTagged" );
  
  if (input.muonType() == xAOD::Muon::SiliconAssociatedForwardMuon) {
    ////ATH_MSG_FATAL("MuonType == xAOD::Muon::SiliconAssociatedForwardMuon" );
  ...
  //if (input.muonType() == xAOD::Muon::Combined || input.muonType() == xAOD::Muon::MuonStandAlone ||  input.muonType() == xAOD::Muon::SegmentTagged ||  input.muonType() == xAOD::Muon::CaloTagged) { 
    track = input.primaryTrackParticle();
    //}
    //if (!track) return StatusCode::FAILURE; 
  }
	//ATH_MSG_FATAL("B4");
	//ATH_MSG_FATAL("Track " << track);
	////ATH_MSG_FATAL("After");
  //impact parameters (after applyCorrection() so to have the primaryTrack links restored in old buggy samples)
  try{
   if (track != 0) dec_z0sinTheta(input) = (track->z0() + track->vz() - primvertex_z) * TMath::Sin(input.p4().Theta());
  }
   catch (...) {
   float z0sigError = -99.; 
    ATH_MSG_WARNING("FillMuon : Exception  Setting dummy decoration z0sig=" << z0sigError );
    dec_z0sinTheta(input) =z0sigError;
   }
   //ATH_MSG_FATAL("After z0Sig");
 
  //protect against exception thrown for null or negative d0sig
  try {
    if (track != 0) dec_d0sig(input) = xAOD::TrackingHelpers::d0significance( track , evtInfo->beamPosSigmaX(), evtInfo->beamPosSigmaY(), evtInfo->beamPosSigmaXY() );
  }
  catch (...) {
    float d0sigError = -99.; 
...
bool SUSYObjDef_xAOD::IsCosmicMuon(const xAOD::Muon& input, float z0cut, float d0cut) const
{
  ...
  //ATH_MSG_FATAL("CosmicMu_track " << track);

  //try{
  	double mu_d0 = -99.;
  	if (track != 0) mu_d0 = track->d0();
  	//else {
  	//	double mu_d0 = -99.;
  	//	}
  	//}
  ...
  //ATH_MSG_FATAL("CosmicMu_d0 " << mu_d0);
  
  const xAOD::Vertex* pv = this->GetPrimVtx();
  double primvertex_z = pv ? pv->z() : 0;
  
  //try {
  	double mu_z0_exPV = -99.;
  	if (track != 0) mu_z0_exPV = track->z0() + track->vz() - primvertex_z;
  	//else {
  	//	double mu_z0_exPV = -99.;
  	//	}
  	//}
  ...
  //double mu_z0_exPV = -99.;
  //}
  //ATH_MSG_FATAL("CosmicMu_z0_exPV " << mu_z0_exPV);
  
  bool isCosmicMuon = false;

   if (mu_d0 != -99. && mu_z0_exPV != -99. ) {
   	isCosmicMuon = ( fabs(mu_z0_exPV) >= z0cut || fabs(mu_d0) >= d0cut);  
   }
   else {
   	isCosmicMuon = true;
   }
   
   //ATH_MSG_FATAL("IsCosmicMuon " << isCosmicMuon );
