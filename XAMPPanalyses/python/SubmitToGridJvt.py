...
if __name__=='__main__':
        ...
        parser.add_argument('-i','--inputDS',help='input dataset or DS list present in \"SUSYFwMPI/data/mc15_13TeV/SampleLists/\"',required=True)
        ...
        parser.add_argument('-a','--analysis',help='choose the analysis to run, for Custom (default) you have to setup the cutflow the others have a std cutflow that can be used',choices=['Custom','Stop0L','Stop0L_SR','Stop0L_1LCR','Stop0L_2LCR','JVT_2L'],default='Custom')
        ...
        InDSListDir = "%s/data/SUSYFwMPI/SampleLists/%s/"%(ROOTCOREBIN, Campaign)
...
#                 InDSList = InDSListDir+InDSList
        ...
        if RunOptions.outDS == '':
                if len(InputDataSets)==1:
                        ...
                        OutDS = "user.%s.%s.SUSYFwMPI.%s"%(RUCIO,TODAY,JobName)
        else:
...
#         print ConvertListToString(PRUN_Options , " ")
        Command = "prun --exec=\"python SUSYFwMPI/util/runJvt.py %s\" %s "%(ConvertListToString (AnalysisOptions, " ") , ConvertListToString(PRUN_Options , " ") )
